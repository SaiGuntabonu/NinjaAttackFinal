...
class GameScene: SKScene {
  ...
  let velocityMultipler: CGFloat=0.2
  var monstDuration = 3.0
  ...
  override func didMove(to view: SKView) {
    ...
    let analogJoystick: AnalogJoystick = {
      ...
      js.position = CGPoint(x: size.width * 0.9, y: js.radius+10 )
    ...
    func setupJoystick() {
       ...
       analogJoystick.trackingHandler = { [unowned self] data in
        if player.position.y<size.height && player.position.y>0 {
          self.player.position = CGPoint(x: player.position.x , y: player.position.y + (data.velocity.y*velocityMultipler))
        } else {
   ...
   player.position = CGPoint(x: size.width * 0.1, y: size.height * 0.5)
    ...
    run(SKAction.repeatForever(
      SKAction.sequence([
        ...
        SKAction.wait(forDuration:monstDuration)
      ])
    ))
  ...
  override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
    ...
    let realDest = CGPoint(x: size.width * 1.5, y: player.position.y )
